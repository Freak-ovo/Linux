...
void LCD_DrawLine(unsigned short x1, unsigned short y1, unsigned short x2, unsigned short y2)
{
	...
	{  
		...
		LCD_DrawPoint(uRow, uCol, TFT_LCD_DEV.ForeColor);/* 画点 */
		xerr += delta_x ; 
		yerr += delta_y ; 
		if(xerr > distance) 
		{ 
...
void LCD_Draw_Circle(unsigned short x0,unsigned short y0,unsigned char r)
{
    ...
    {
        LCD_DrawPoint(x  + mx, y  + my, TFT_LCD_DEV.ForeColor);
        LCD_DrawPoint(y  + mx, x  + my, TFT_LCD_DEV.ForeColor);
        LCD_DrawPoint(-x + mx, y  + my, TFT_LCD_DEV.ForeColor);
        LCD_DrawPoint(-y + mx, x  + my, TFT_LCD_DEV.ForeColor);
...
{  							  
	...
	{   
		...
		for(t1 = 0; t1 < 8; t1++)
		{
			if(temp & 0x80) LCD_DrawPoint(x, y, TFT_LCD_DEV.ForeColor);
			else if(mode==0) LCD_DrawPoint(x, y, TFT_LCD_DEV.BackColor);
			temp <<= 1;
			y++;
