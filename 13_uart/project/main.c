...
int main(void)
{
    ...
    while (1)
    {
        UART_WriteString(UART1, (const unsigned char *)"请输入一个字符!\r\n");  /* 通过UART1发送字符串 */

        ch = UART_Read(UART1);  /* 从UART1读取一个字符 */

        UART_WriteString(UART1, (const unsigned char *)"\r\n");  /* 通过UART1发送换行符 */

        UART_WriteString(UART1, (const unsigned char *)"您输入的字符是: ");
        UART_WriteString(UART1, &ch);  /* 通过UART1发送字符 */
        UART_WriteString(UART1, (const unsigned char *)"\r\n");  /* 通过UART1发送换行符 */
